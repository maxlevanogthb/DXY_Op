...
function verificarSubtipo() {
    ...
    const nombreCategoria = opcion.dataset.nombre ? opcion.dataset.nombre.toLowerCase() : "";

    if (nombreCategoria.includes("armazón") || nombreCategoria.includes("armazon")) {
        ...
        columns: [
            ...
            // COL 3: DETALLES (Badge azul suave + info limpia)
            {
                ...
                render: (data, type, row) => {
                    ...
                    const colorInfo = row.color ? `<span class="text-dark fw-medium">${row.color}</span>` : '<span class="text-muted fst-italic">Sin color</span>';
                    const tallaInfo = row.talla ? `<span class="ms-2 ps-2 border-start border-secondary">${row.talla}</span>` : '';
                    const subTipoInfo = row.subTipo ? `<div class="mt-1 small text-muted"><i class="fas fa-glasses me-1 text-primary opacity-50"></i>${row.subTipo}</div>` : '';
            ...
            {
                data: "precioVenta",
                className: "text-end align-middle fw-bold text-success font-monospace py-3 fs-6",
                render: (precio) => {
...
function inicializarFiltros() {
    ...
    $("#buscarNombre").on("keyup", debounce(function () {
...
function editarProducto(id) {
    fetch(APIURL + "/" + id)
        ...
        .then(prod => {
            ...
            // Corrección: Usamos 'prod.precio' que vimos en tus logs
            $("#precio").val(prod.precio || prod.precioVenta);

            const catId = (prod.tipo && typeof prod.tipo === "object") ? prod.tipo.id : prod.tipo;
            $("#categoria").val(catId);
            
            verificarSubtipo(); // Importante
